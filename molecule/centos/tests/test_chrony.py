...
def test_chrony_client(host, chronys):
    chronyconf = host.file('/etc/chrony.conf')
    assert chronyconf.exists
    assert chronyconf.user == 'root'
    assert chronyconf.group == 'root'
    assert chronyconf.contains("server %s.be.pool.ntp.org" % chronys)
...
def test_ntp_service(host):
    assert host.service('ntpd').is_enabled is False
    assert host.service('ntpd').is_running is False
